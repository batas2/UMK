...
procedure Kruskal(var Korzen, Lisc: TWierzcholki;
      ...
      {Sprawdzamy czy wierzcholek rowny poczatkowni krawendzi lezy w tym samym zbiorze co
       wierzcholek odpowiadajacy koncowi krawendzi, za pomoca reprezentantow zbiorow (find_set)}
        begin
          ...
          //Koncowi dluzszej listy przypisuje poczatek krotszej
...
procedure PostOrder(var DrzewoRozp: TDrzewoRozpinajace; i: Integer);
var
  j: Integer;
begin

  for j := 0 to Length(DrzewoRozp[i]) - 1 do
    PostOrder(DrzewoRozp, DrzewoRozp[i][j]);
...
begin
  ...
  Kruskal(Korzen, Lisc, Krawendz, DrzewoRozp, m);

  Write('0');
  PostOrder(DrzewoRozp, 0);
  Writeln;
